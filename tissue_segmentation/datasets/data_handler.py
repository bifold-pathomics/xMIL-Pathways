...
class SlideDataHandler:
    ...
    @staticmethod
    def load_patch_metadata(slide_metadata, patches_dirs, patch_filters, annotation_label_fn=None):
        """
        ...
        :param annotation_label_fn: (function) A function mapping a patch annotation dict to a label.
        :return: (list, list, list) Per slide: tensor of indices to filter the feature vectors per slide, tensor of
            patch ids, and tensor of patch labels if an annotation_label_fn is given.
            Ordered by the given slide barcodes.
        """
        print("Loading patch metadata")
        feature_indices, patch_ids, patch_labels = [], [], []
        for idx, row in tqdm(slide_metadata.iterrows(), total=len(slide_metadata)):
            ...
            if annotation_label_fn is not None:
                labels = slide_patches['annotation_classes'].apply(annotation_label_fn)
                # Exclude patches without label
                slide_patches = slide_patches[~labels.isna()]
                patch_labels.append(torch.tensor(labels[~labels.isna()].astype(int).values))
        ...
        return feature_indices, patch_ids, patch_labels
