...
def test_classification_model(
        model, classifier, dataloader_test, callback, label_cols, tb_writer=None, return_args=False, verbose=False):
    model.eval()
    ...
    for i_batch, batch in enumerate(tqdm(dataloader_test)):
        ...
        prob_pred_val, labels_val, loss, sample_ids = classifier.validation_step(batch)

        all_preds.append(prob_pred_val)
        all_labels.append(labels_val)
        loss_test.append(loss.item())
        if isinstance(sample_ids, dict):
            all_sample_ids = {key: all_sample_ids.get(key, []) + _tolist(ids) for key, ids in sample_ids.items()}
        else:
            all_sample_ids = {'sample_id': all_sample_ids.get('sample_id', []) + _tolist(sample_ids)}
        torch.cuda.empty_cache()
    ...
    results = callback.save_test_results(auc_test, loss_test, all_preds, all_labels, label_cols,
                                         all_sample_ids, return_args=return_args)
