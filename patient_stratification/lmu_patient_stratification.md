...
# Table S1 - Clinicopathological parameters of LMU cohort - categorial parameters
...
Helper function to generate Latex table
    ...
    clin_to_latex(clin, exclude = c(1,2,3,14:17))
...
# Table S2 - Clinicopathological parameters of LMU cohort - continous parameters
...
predictive LRP-positive fractions are representative for the slide.
    ...
    df_for_mat <- pred %>%
      ...
      select(slide_id, pathway, TAPAS, case_id, location) %>%
      spread(key="pathway", value="TAPAS") 
...
# Figure S9a
    ...
    df_for_mat_sl <- pred %>%
      filter(num_tumor_patches>100) %>%
      select(slide_id, pathway, slide_level_prediction_score, case_id, location) %>%
      spread(key="pathway", value="slide_level_prediction_score") 
    mat_sl <- apply(as.matrix(df_for_mat_sl[,-c(1:3)]),2,as.numeric)
    rownames(mat_sl) <- df_for_mat_sl$slide_id

    column_annotation <- HeatmapAnnotation(
      type=df_for_mat_sl$location,
      col = list(
        type=c("P"="grey80","LMET"="#117733")
      )
    )
    ...
    pdf("plots/figureS9a.pdf", width = 7, height = 3)
    set.seed(1)
    ht_drawn_sl <- draw(ht_sl)
    dev.off()
...
# Figure 3b - Waterfall plot of number of slides per resection specimen
    ...
    patients_cluster_df$Freq[patients_cluster_df$Var2=="metabolic persistence"] <- patients_cluster_df$Freq[patients_cluster_df$Var2=="metabolic persistence"] * (-1)
    ...
    patients_cluster_df %>%
      ...
      scale_fill_manual(values = c("metabolic persistence" = "#0072B2",
...
![](lmu_patient_stratification_files/figure-markdown_strict/unnamed-chunk-9-1.png)
...
![](lmu_patient_stratification_files/figure-markdown_strict/unnamed-chunk-10-1.png)
